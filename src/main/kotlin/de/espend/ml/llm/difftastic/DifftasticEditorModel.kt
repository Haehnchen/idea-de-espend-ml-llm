...
data class DifftasticUnifiedModel(
    ...
    val sourceLines: List<DifftasticSourceLine?>,
)
...
data class DifftasticEditorSide(
    ...
    val lineMarkers: List<DifftasticLineMarker>,
    val tokenMarkers: List<DifftasticTokenMarker>,
    val displayToOriginalLine: List<Int?>,
)

data class DifftasticSourceLine(
    val side: DifftasticSourceSide,
    val line: Int,
)

enum class DifftasticSourceSide { LEFT, RIGHT }
...
object DifftasticEditorModelBuilder {
    ...
    fun buildUnified(input: DifftasticEditorInput, collapseUnchanged: Boolean = false): DifftasticUnifiedModel {
        ...
        val sourceLines = mutableListOf<DifftasticSourceLine?>()
        var displayLine = 0
        ...
        fun appendLine(
            ...
            sourceLine: DifftasticSourceLine?,
        ) {
            if (text.isNotEmpty()) {
                text.append('\n')
            }
        ...
        rows.forEach { row ->
            when (row.kind) {
                RowKind.Context -> appendLine(
                    "  ",
                    row.rightText.ifEmpty { row.leftText },
                    null,
                    emptyList(),
                    row.rightLineNumber?.let { DifftasticSourceLine(DifftasticSourceSide.RIGHT, it) }
                        ?: row.leftLineNumber?.let { DifftasticSourceLine(DifftasticSourceSide.LEFT, it) },
                )
                RowKind.Inserted -> appendLine(
                    "+ ",
                    row.rightText,
                    DifftasticLineKind.Inserted,
                    row.rightChanges,
                    row.rightLineNumber?.let { DifftasticSourceLine(DifftasticSourceSide.RIGHT, it) },
                )
                RowKind.Removed -> appendLine(
                    "- ",
                    row.leftText,
                    DifftasticLineKind.Removed,
                    row.leftChanges,
                    row.leftLineNumber?.let { DifftasticSourceLine(DifftasticSourceSide.LEFT, it) },
                )
                RowKind.Modified -> {
                ...
                RowKind.Collapsed -> appendLine(
                    "  ",
                    row.rightText,
                    DifftasticLineKind.Collapsed,
                    emptyList(),
                    null,
                )
            }
    ...
    private fun buildSide(rows: List<ModelRow>, isLeft: Boolean): DifftasticEditorSide {
        ...
        val lineMarkers = mutableListOf<DifftasticLineMarker>()
        val tokenMarkers = mutableListOf<DifftasticTokenMarker>()
        val displayToOriginalLine = mutableListOf<Int?>()
        ...
        rows.forEachIndexed { displayLine, row ->
            ...
            displayToOriginalLine += originalLine
        ...
        return DifftasticEditorSide(
            ...
            lineMarkers = lineMarkers,
            tokenMarkers = tokenMarkers,
            displayToOriginalLine = displayToOriginalLine,
        )
    }
        ...
        if (status == "unchanged") {
            ...
            return (0 until maxLineCount).map { index ->
                ModelRow(
                    ...
                    leftText = leftLines.getOrNull(index).orEmpty(),
                    leftChanges = emptyList(),
                    rightLineNumber = index.takeIf { index < rightLines.size },
                    rightText = rightLines.getOrNull(index).orEmpty(),
