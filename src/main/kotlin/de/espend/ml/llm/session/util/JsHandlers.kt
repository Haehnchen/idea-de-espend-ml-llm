...
object JsHandlers {
    ...
    fun generateExpandScript(): String {
        return buildString {
            ...
            appendLine("}")
            appendLine("")
            appendLine("function toggleToolCallGroup(button) {")
            appendLine("    var group = button.closest('.tool-call-group');")
            appendLine("    if (!group) return;")
            appendLine("    var isExpanded = group.classList.contains('expanded');")
            appendLine("    group.classList.toggle('expanded', !isExpanded);")
            appendLine("    group.classList.toggle('collapsed', isExpanded);")
            appendLine("    button.setAttribute('aria-expanded', String(!isExpanded));")
            appendLine("}")
            appendLine("")
